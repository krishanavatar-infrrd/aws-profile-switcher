        ...
        if config_file.exists():
            try:
                with open(config_file, 'r') as f:
                    config = json.load(f)
                self.base_credentials_path = Path(
                    config.get('base_credentials_path', '/home/krishnavatar/Downloads/credentials'))
        ...
        if self.credentials_path.exists():
            ...
            if 'infrrd-master' in config.sections():
                if self.is_profile_valid(config['infrrd-master']):
                    print(
                        f"✅ infrrd-master profile: Valid (Access Key: {config['infrrd-master']['aws_access_key_id'][:10]}...)")
                else:
            ...
            try:
                ...
                if base_creds:
                    ...
                    if ('default' in config.sections() and 'infrrd-master' in config.sections() and
                            self.is_profile_valid(config['default']) and self.is_profile_valid(
                                config['infrrd-master'])):
